	...
	internal class XdrStream : Stream
	{
		...
		public async Task<byte[]> ReadBytesAsync(int count, CancellationToken cancellationToken)
		{
			...
			if (count > 0)
			{
				...
				while (toRead > 0 && currentlyRead != 0)
				{
					toRead -= (currentlyRead = await this.ReadAsync(buffer, count - toRead, toRead).ConfigureAwait(false));
				}
				if (toRead == count)
				{
					throw new IOException();
		...
		public async Task<byte[]> ReadOpaqueAsync(int length, CancellationToken cancellationToken)
		{
			...
			if (padLength > 0)
			{
				await this.ReadAsync(Pad, 0, padLength, cancellationToken);
			}
		...
		{
			return this.ReadOpaqueAsync((ushort)this.ReadInt32(), cancellationToken);
		}
		...
		{
			return this.ReadStringAsync(charset, this.ReadInt32(), cancellationToken);
		}
		...
		{
			this.Write(BitConverter.ToInt32(BitConverter.GetBytes(value), 0));
		}
		public Task WriteAsync(float value, CancellationToken cancellationToken)
		{
			return this.WriteAsync(BitConverter.ToInt32(BitConverter.GetBytes(value), 0), cancellationToken);
		}
		...
		{
			return this.WriteAsync(BitConverter.ToInt64(BitConverter.GetBytes(value), 0), cancellationToken);
		}
		...
		public async Task WriteAsync(decimal value, int type, int scale, CancellationToken cancellationToken)
		{
			...
			{
				case IscCodes.SQL_SHORT:
					await this.WriteAsync((short)numeric, cancellationToken).ConfigureAwait(false);
					break;

				case IscCodes.SQL_LONG:
					await this.WriteAsync((int)numeric, cancellationToken).ConfigureAwait(false);
					break;

				case IscCodes.SQL_QUAD:
				case IscCodes.SQL_INT64:
					await this.WriteAsync((long)numeric, cancellationToken).ConfigureAwait(false);
					break;

				case IscCodes.SQL_DOUBLE:
				case IscCodes.SQL_D_FLOAT:
					await this.WriteAsync((double)value, cancellationToken).ConfigureAwait(false);
					break;
			}
		}
			...
			try
			{
				if (param.DbDataType != DbDataType.Null)
				{
					...
					switch (param.DbDataType)
					{
						...
						case DbDataType.Decimal:
						case DbDataType.Numeric:
							await this.WriteAsync(param.DbValue.GetDecimal(), param.DataType, param.NumericScale, cancellationToken).ConfigureAwait(false);
							break;
